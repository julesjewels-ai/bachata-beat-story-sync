...
@patch('cv2.resize')
@patch('cv2.imencode')
def test_extract_thumbnail_success(mock_imencode, mock_resize, analyzer):
    """Test successful thumbnail extraction from a video."""
    # Setup Mock Capture
    mock_cap = MagicMock()
    ...
    # Call
    result = analyzer._extract_thumbnail(mock_cap)
...
def test_extract_thumbnail_empty(analyzer):
    """Test thumbnail extraction returns None with 0 frames."""
    mock_cap = MagicMock()
...
def test_extract_thumbnail_read_fail(analyzer):
    """Test thumbnail extraction returns None when frame read fails."""
    mock_cap = MagicMock()
