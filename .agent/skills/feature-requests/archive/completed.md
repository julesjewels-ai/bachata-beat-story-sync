...
| FEAT-038 | Quick Preview Plan Visibility | `IMPLEMENTED` | In Quick Preview (dry-run) mode, move the generated segment plan to the top of the output section so users see it immediately after the run completes, rather than having to scroll past other content |
| FEAT-039 | Stable Progress Expander During Refresh | `IMPLEMENTED` | Fix the progress expander (e.g. "Analyzing clip 1 of 4") collapsing on every Streamlit rerun. The dropdown should stay open while a task is in progress; investigate using `st.empty()` containers or session-state-driven expansion state to prevent collapse on refresh |
| FEAT-040 | Disable UI Controls During Active Processing | `IMPLEMENTED` | All sidebar widgets, input components, and action buttons disabled via `disabled=state.is_running`. Generate Montage button replaced with a Cancel button during active runs; cancel sets `is_running=False` and displays an error message (background thread completes silently). |
| FEAT-045 | Text Overlay Infrastructure | `IMPLEMENTED` | `src/core/text_overlay.py`: `TextEvent` dataclass, `build_drawtext_filter_chain()`, `parse_lrc()`, `lrc_to_text_events()`, `escape_drawtext()`, `resolve_font()`. Three styles: `wash` (10% opacity, centered), `lower_third` (clean subtitle with 40% black box). Spanish UTF-8 safe. `PacingConfig` fields: `text_overlay_enabled`, `text_overlay_font`, `text_overlay_color`, `lyrics_overlay_enabled`, `lyrics_lrc_path`, `cold_open_enabled`, `track_artist`, `track_title`. `apply_text_overlay()` in `ffmpeg_renderer.py`. Bundled font README at `assets/fonts/`. |
| FEAT-046 | Cinematic Cold Open | `IMPLEMENTED` | `build_cold_open_events()` in `text_overlay.py`. Sequence: 0–4s wash scene-setter, 4–7s artist/title lower-third. Scene text priority: (1) `{stem}.scene.txt` sidecar, (2) most-repeated lyric from `{stem}.lrc` via `_most_repeated_lrc_line()`, (3) silent skip. Demo sidecar at `demo/audio/sample_bachata.scene.txt`. 33 unit tests in `tests/unit/test_text_overlay.py`. |
