...
def _build_timeline_svg(
    ...
    lines.append(
        f'<line x1="{margin}" y1="{axis_y}" x2="{width - margin}" y2="{axis_y}" stroke="#999" stroke-width="1"/>'
    )
    ...
    lines.append(
        f'<rect x="{margin}" y="{legend_y}" width="12" height="12" fill="#ef4444" stroke="#333" stroke-width="1"/>'
    )
    ...
    lines.append(f'<text x="{margin + 166}" y="{legend_y + 10}">Medium Intensity</text>')
    lines.append(
...
def _build_decision_table(decisions: list[SegmentDecision]) -> str:
    ...
    """
    rows = []
    rows.append("<table style='width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 14px;'>")
    rows.append("<thead style='background: #f3f4f6; border-bottom: 2px solid #9ca3af;'>")
    rows.append(
...
def _build_stats_summary(
    ...
    rows = ["<div style='display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 20px; margin: 30px 0;'>"]
...
def _build_config_summary(config: PacingConfig) -> str:
    ...
    rows.append(f"<li>Min clip duration: <strong>{config.min_clip_seconds}s</strong></li>")
    rows.append(
    ...
    rows.append(f"<li>Max clips: <strong>{config.max_clips or 'unlimited'}</strong></li>")
    rows.append(f"<li>Max duration: <strong>{config.max_duration_seconds or 'unlimited'}s</strong></li>")
    ...
    if config.intro_effect and config.intro_effect != "none":
        rows.append(f"<li>Intro effect: <strong>{config.intro_effect} ({config.intro_effect_duration}s)</strong></li>")

    if config.transition_type and config.transition_type != "none":
        rows.append(f"<li>Transitions: <strong>{config.transition_type} ({config.transition_duration}s)</strong></li>")
