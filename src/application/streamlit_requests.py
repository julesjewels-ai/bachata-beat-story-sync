...
def _apply_style_settings(
    settings: GenerationSettings, pacing_kwargs: dict[str, Any]
) -> None:
...
def _apply_demo_overrides(demo_dry_run: bool, pacing_kwargs: dict[str, Any]) -> None:
    if demo_dry_run:
...
def _apply_speed_ramp_settings(
    settings: GenerationSettings, pacing_kwargs: dict[str, Any]
) -> None:
...
def _apply_text_overlay_settings(
    settings: GenerationSettings, pacing_kwargs: dict[str, Any]
) -> None:
...
def build_pacing_kwargs(
    ...
    *,
    ...
    _apply_style_settings(settings, pacing_kwargs)

    if demo_mode:
        _apply_demo_overrides(demo_dry_run, pacing_kwargs)
    elif settings.dry_run:
        pacing_kwargs["dry_run"] = True

    _apply_speed_ramp_settings(settings, pacing_kwargs)
    _apply_visual_effects(settings, pacing_kwargs)

    if not demo_mode:
        _apply_text_overlay_settings(settings, pacing_kwargs)
        _apply_limit_settings(settings, pacing_kwargs)
