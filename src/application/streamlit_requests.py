...
def _apply_demo_settings(demo_dry_run: bool, pacing_kwargs: dict[str, Any]) -> None:
    if demo_dry_run:
        pacing_kwargs["dry_run"] = True
    pacing_kwargs.update(
        {
            "max_clips": 6,
            ...
            "track_title": "Sample Bachata",
        }
    )


def _apply_speed_settings(
    settings: GenerationSettings, pacing_kwargs: dict[str, Any]
) -> None:
...
def build_pacing_kwargs(
    ...
    *,
    ...
    if demo_mode:
        _apply_demo_settings(demo_dry_run, pacing_kwargs)
    elif settings.dry_run:
        pacing_kwargs["dry_run"] = True

    _apply_speed_settings(settings, pacing_kwargs)

    if not demo_mode:
        _apply_text_settings(settings, pacing_kwargs)
        _apply_duration_settings(settings, pacing_kwargs)
