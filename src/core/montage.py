...
class MontageGenerator:
    ...
    def _extract_segments(
        ...
        for i, seg in enumerate(segments):
            ...
            if config.video_style != "none":
                if config.video_style == "bw":
                    vf_parts.append("hue=s=0")
                elif config.video_style == "vintage":
                    vf_parts.append("curves=vintage,vignette")
                elif config.video_style == "warm":
                    vf_parts.append("colorchannelmixer=rr=1.2:gg=1.1:bb=0.9")
                elif config.video_style == "cool":
                    vf_parts.append("colorchannelmixer=rr=0.9:gg=1.1:bb=1.2")
