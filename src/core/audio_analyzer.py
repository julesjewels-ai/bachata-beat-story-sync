...
def detect_sections(
    ...
    boundaries_int = [int(b) for b in sorted(set(boundaries))]

    final_boundaries = _merge_short_boundaries(boundaries_int)

    sections: list[MusicalSection] = []
    for i in range(len(final_boundaries) - 1):
        start_idx = final_boundaries[i]
        end_idx = final_boundaries[i + 1]
        ...
        label = _determine_section_label(
            i, len(final_boundaries), avg_intensity, start_idx, end_idx, smoothed
        )
