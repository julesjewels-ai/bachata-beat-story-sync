...
def _build_pacing_filters_for_segment(
    ...
    If seg.phase_pacing_effects is None, falls through to global
    render_config flags.
    If seg.phase_pacing_effects is a list (even empty), only those named
    effects are active.
...
def build_overlay_filter(
    ...
    return (
        f"[1:a]{src_filter}{post}[viz];[0:v][viz]overlay={x_expr}:H-h-{pad}{enable_expr}[outv]"
    )
