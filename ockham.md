...
**Summary:** Refactored the `validate_file_path` function by extracting the directory resolution logic into a separate `_resolve_directory_file` helper function and pre-computing a lowercase Set for allowed extensions. The original function signature and semantic behavior were retained.
